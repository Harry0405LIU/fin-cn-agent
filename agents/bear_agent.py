#!/usr/bin/env python3
"""
Bear Agent - 唱空Agent（巴菲特 persona）
专注于识别股票的风险因素和看跌理由
"""
...
class BearAgent:
    """唱空Agent - 寻找和强调股票的风险因素（巴菲特 persona）"""

    # 内置 fallback persona（当 skills/persona 文件缺失时使用）
    _FALLBACK_PERSONA = """你是沃伦·巴菲特，以空头/质疑方身份分析这只股票。你的核心纪律：安全边际——当前价格是否低于保守估值20%以上？好公司买贵了同样是坏投资。能力圈——这门生意你能否用一句话说清？10年后大概率还在做？镜子测试五维——生意本质/护城河/管理层/估值/安全边际，任一维度不及格即否决。管理层——资本配置记录如何？有诚信硬伤吗？护城河——定价权、转换成本、网络效应、规模优势，哪一个在变窄？你的结论倾向：PASS（能力圈外）/ HOLD-DO NOT ADD / REDUCE。所有判断必须基于提供的财务数据；数据不足时标注「数据不足」而非臆造。"""
    ...
    def _load_persona(self, persona_path: Optional[str] = None) -> str:
        ...
        if persona_path is None:
            # 自动查找：skills/persona/buffett-bear.md
            candidates = [
                Path(__file__).resolve().parent.parent / "skills" / "persona" / "buffett-bear.md",
                Path("skills/persona/buffett-bear.md"),
            ]
        else:
        ...
        print(f"  ⚠ 巴菲特 persona 文件未找到，使用内置精简版")
...
请针对上述看多观点进行质疑和反驳，并提供你的看空论据。
"""

        user_prompt += """
请以JSON格式返回分析结果，包含以下字段：
{
    "summary": "整体看空观点总结（1-2句话）",
    "key_bearish_points": [
        "看空观点1",
        "看空观点2",
        ...
    ...
    "dimension_scores": {
        "company_quality": {
            "score": -3.5,
            "reasoning": "商业模式脆弱，现金流不稳定，业务复杂度高",
            "sub_scores": {
                "business_model": -4.0,
                "corporate_culture": -3.0,
                "understandability": -3.5
            },
            "is_friend_of_time": false
        },
        "trend": {
            "score": -2.0,
            "reasoning": "行业景气度下行，公司增长放缓",
            "sub_scores": {
                "industry_trend": -2.5,
                "company_trend": -2.0,
                "sentiment_trend": -1.5
        ...
        "valuation": {
            "score": -6.0,
            "reasoning": "估值严重偏高，缺乏安全边际",
            "sub_scores": {
                "relative_valuation": -6.5,
                "absolute_valuation": -6.0,
                "safety_margin": -5.5
            }
        }
    }
...
"""
    ...
    def rebut(
        ...
        Returns:
            反驳内容
        """
        system_prompt = """你是一位专业的看空分析师，面对看多分析师的乐观观点，你需要：
1. 仔细分析对方的观点
2. 基于事实和数据指出对方过于乐观的地方
3. 强调对方可能忽略的风险和问题
4. 重申看空的核心论据
5. 保持专业和理性，避免情绪化
